...
export default function RatingsPage() {
  ...
  useEffect(() => {
    async function getRatings(token: string) {
      setIsLoading(true);
      const filmList: FilmRatingsType[] = await fetchFilmRatings(token);
      ...
      filmList.forEach((film) => {
        dispatch(ratingsSlice.actions.addToRatings(film));
      });
      setIsLoading(false);
    }
  ...
  async function fetchFilmRatings(token: string): Promise<FilmRatingsType[]> {
    try {
      const response = await getFilmRatings(token);
